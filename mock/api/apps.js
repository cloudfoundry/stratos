...
function init(router, config) {
  ...
  router.get('/pp/v1/proxy/v2/apps', function (request, response) {
...
function mockAppsResponse(request, config) {
  ...
  _.each(cnsiList, function (cnsi) {
    ...
    cnsiResponse.resources = generateApps(cnsiApps[cnsi], currentPage, appsPerPage);
...
function generateApps(totalResults, currentPage, appsPerPage) {
