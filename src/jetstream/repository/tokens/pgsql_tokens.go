...
// TODO: RC
var findAllCNSIToken2 = `SELECT token_guid, auth_token, refresh_token, token_expiry, disconnected, auth_type, meta_data, user_guid, linked_token
										FROM tokens
										WHERE cnsi_guid = $1 AND token_type = 'cnsi'`

var findAllCNSITokenConnected = `SELECT token_guid, auth_token, refresh_token, token_expiry, disconnected, auth_type, meta_data, user_guid, linked_token
										FROM tokens
										WHERE cnsi_guid = $1 AND token_type = 'cnsi' AND disconnected = '0'`
...
func InitRepositoryProvider(databaseProvider string) {
	...
	findAllCNSIToken2 = datastore.ModifySQLStatement(findAllCNSIToken2, databaseProvider)
	findAllCNSITokenConnected = datastore.ModifySQLStatement(findAllCNSITokenConnected, databaseProvider)
...
// TODO: RC remove all `all`
func (p *PgsqlTokenRepository) FindAllCNSIToken(cnsiGUID string, encryptionKey []byte) ([]interfaces.TokenRecord, error) {
	log.Debug("FindAllCNSIToken")
	return p.findAllCNSIToken(cnsiGUID, encryptionKey, false)
}

func (p *PgsqlTokenRepository) FindAllCNSITokenIncludeDisconnected(cnsiGUID string, encryptionKey []byte) ([]interfaces.TokenRecord, error) {
	log.Debug("FindAllCNSITokenIncludeDisconnected")
	return p.findAllCNSIToken(cnsiGUID, encryptionKey, true)
}

func (p *PgsqlTokenRepository) findAllCNSIToken(cnsiGUID string, encryptionKey []byte, includeDisconnected bool) ([]interfaces.TokenRecord, error) {
	log.Debug("findAllCNSIToken")
	if cnsiGUID == "" {
		msg := "Unable to find CNSI Token without a valid CNSI GUID."
		log.Debug(msg)
		return make([]interfaces.TokenRecord, 0), errors.New(msg)
	}
	...
	if includeDisconnected {
		...
		rows, err = p.db.Query(findAllCNSITokenConnected, cnsiGUID)
	}
	if err != nil {
	...
	trs := make([]interfaces.TokenRecord, 0)
	for rows.Next() {
		// temp vars to retrieve db data
		var (
			tokenGUID              sql.NullString
		...
		err = rows.Scan(&tokenGUID, &ciphertextAuthToken, &ciphertextRefreshToken, &tokenExpiry, &disconnected, &authType, &metadata, &tokenUserGUID, &linkedTokenGUID)
		if err != nil {
	...
	// TODO: RC merge with find single
	// TODO: RC Use?
...
// func (p *PgsqlTokenRepository) Overwrite(tr interfaces.TokenRecord, encryptionKey []byte) error {
// 	log.Debug("Overwrite Token")

// 	if _, err := p.FindCNSITokenIncludeDisconnected(endpointGuid, userGuid, encryptionKey); err != nil {
// 		// Found, so update endpoint
// 		// TODO: RC ALL STRINGS?
// 		return p. Update(endpoint, encryptionKey)
// 	} else {
// 		// Not Found, create endpoint
// 		return p.SaveCNSIToken(endpointGuid, userGuid, tr, encryptionKey)
