...
func SyncRepo(store ChartRepoDatastore, repoName, repoURL string, authorizationHeader string) error {
	...
	for i := 0; i < numWorkers; i++ {
		wg.Add(1)
		go importWorker(store, &wg, iconJobs, chartFilesJobs)
	}
...
func importWorker(store ChartRepoDatastore, wg *sync.WaitGroup, icons <-chan Chart, chartFiles <-chan importChartFilesJob) {
	defer wg.Done()
	for c := range icons {
		log.WithFields(log.Fields{"name": c.Name}).Debug("importing icon")
		if err := fetchAndImportIcon(store, c); err != nil {
	...
	for j := range chartFiles {
		...
		if err := fetchAndImportFiles(store, j.Name, j.Repo, j.ChartVersion); err != nil {
			log.WithFields(log.Fields{"name": j.Name, "version": j.ChartVersion.Version}).WithError(err).Error("failed to import files")
		}
	}
}

func fetchAndImportIcon(store ChartRepoDatastore, c Chart) error {
	if c.Icon == "" {
	...
	return store.StoreChartIcon(c, b)
}

func fetchAndImportFiles(store ChartRepoDatastore, name string, r Repo, cv ChartVersion) error {
	...
	return store.StoreChartFiles(chartFilesID, chartFiles)
}
