...
describe('CFUserPermissionsService', () => {
  ...
  beforeEach(() => {
    ...
    service = TestBed.get(CFUserPermissionsService);
  });
  ...
  it('should allow create application', done => {
    service.can(CFUserPermissions.APPLICATION_CREATE).pipe(
      tap(can => {
  ...
  it('should allow create application for single endpoint with access', done => {
    service.can(CFUserPermissions.APPLICATION_CREATE, '0e934dc8-7ad4-40ff-b85c-53c1b61d2abb').pipe(
      tap(can => {
        expect(can).toBe(true);
        done();
      }),
  ...
  it('should allow create application for single endpoint with access and org/space', done => {
    service.can(
      CFUserPermissions.APPLICATION_CREATE,
      'c80420ca-204b-4879-bf69-b6b7a202ad87',
      '86577124-4b64-4ca1-9a78-d904c60505c4'
    ).pipe(
      tap(can => {
  ...
  it('should allow if stratos admin', done => {
    service.can(new PermissionConfig(PermissionTypes.STRATOS, CFPermissionStrings.STRATOS_ADMIN)).pipe(
      tap(can => {
        expect(can).toBe(false);
        done();
      }),
