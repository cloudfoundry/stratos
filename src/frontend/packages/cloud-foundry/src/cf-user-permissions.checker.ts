...
export class CFUserPermissionsChecker {
  ...
  public getSimpleCheck(permissionConfig: PermissionConfig, endpointGuid?: string, orgOrSpaceGuid?: string, spaceGuid?: string) {
    switch (permissionConfig.type) {
      ...
      case (PermissionTypes.STRATOS):
        return this.getInternalCheck(permissionConfig.permission as CFPermissionStrings);
  ...
  public getInternalChecks(
    ...
    return configs.map(config => {
      ...
      return this.getInternalCheck(permission as CFPermissionStrings);
    });
  }

  public getInternalCheck(permission: CFPermissionStrings) {
  ...
  private checkAllOfType(endpointGuid: string, type: PermissionTypes, permission: CFPermissionStrings, orgGuid?: string) {
    return this.getEndpointState(endpointGuid).pipe(
      map(state => {
        if (!state || !state[type]) {
          return false;
