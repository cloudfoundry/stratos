...
import {
  CFUserPermissionsChecker as CFUserPermissionsChecker,
  CHECKER_GROUPS,
  IConfigGroup,
  IConfigGroups,
} from './cf-user-permissions.checker';
import {
  CFUserPermissions,
  PermissionConfig,
...
@Injectable()
export class CFUserPermissionsService {
  private checker: CFUserPermissionsChecker;
  constructor(
    store: Store<InternalAppState>
  ) {
    this.checker = new CFUserPermissionsChecker(store);
  }
  /**
   ...
   */
  public can(
    action: CFUserPermissions | PermissionConfigType,
    endpointGuid?: string,
