...
describe('buildDetailTree', () => {
  it('nests orgs → spaces → apps with names, guids and quota links', () => {
    ...
    expect(organization.spaces?.[0]).toMatchObject({ guid: 's1', name: 'space-s1', quota_guid: 'q-space' });
  ...
  it('never exports the space app/route aggregates, which the enrich=none drain leaves at 0', () => {
    const tree = buildDetailTree({
      orgs: [org('o1')],
      spaces: [{ ...space('s1', 'o1'), appCount: 0, routeCount: 0 }],
      apps: [app('a1', { spaceGuid: 's1' })],
    });

    const detailSpace = tree.organizations[0].spaces?.[0];
    expect(detailSpace).not.toHaveProperty('app_count');
    expect(detailSpace).not.toHaveProperty('route_count');
    // The nested list is the trustworthy count, and it disagrees with the 0.
    expect(detailSpace?.apps).toHaveLength(1);
  });
