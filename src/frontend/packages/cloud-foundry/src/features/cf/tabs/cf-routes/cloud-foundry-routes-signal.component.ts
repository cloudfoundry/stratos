    ...
    return [
      {
        ...
        run: (keys: ReadonlySet<string>) => {
          // Only routes with bindings carry work; a no-binding selection
          // can't reach here (action disabled), but filter anyway so the
          // backend call and the count reflect the real targets.
