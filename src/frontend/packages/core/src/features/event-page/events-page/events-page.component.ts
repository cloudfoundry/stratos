...
import { type AppState, getPreviousRoutingState } from "@stratosui/store";
...
import {
	GlobalEventService,
	type IGlobalEvent,
} from "../../../shared/global-events.service";
...
export class EventsPageComponent implements OnInit {
	...
	ngOnInit() {
			...
			map((events: IGlobalEvent[]) => {
				if (this.endpointOnly) {
					return events.filter(
						(event: IGlobalEvent) =>
							event.key.split("-")[0] === "endpointError",
					);
				}
				return events;
			}),
		...
		this.unreadEvents$ = events$.pipe(
			map((events: IGlobalEvent[]) =>
				events.filter((event: IGlobalEvent) => !event.read),
			),
		);
		...
		this.back$ = this.store
			...
			.pipe(first())
			.pipe(
				map((previousState: unknown) =>
					previousState && (previousState as { url: string }).url !== "/login"
						? (previousState as { url: string }).url.split("?")[0]
						: "/home",
				),
	...
	createQueryParams(urlForward: string): Observable<object> {
		...
		return this.back$.pipe(
			map((urlBack: string) => {
				...
				const overrideReturnUrl =
					this.activatedRoute.snapshot.queryParams[eventReturnUrlParam];
				return urlForward?.startsWith("/errors")
					? {
