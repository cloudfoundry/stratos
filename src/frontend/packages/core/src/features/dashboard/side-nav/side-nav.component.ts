...
import { type AppState, Logout, ToggleSideNav } from "@stratosui/store";
...
export class SideNavComponent implements OnInit {
	...
	public navLogoIcon = computed(
		() =>
			this.themeService.theme()?.branding?.navLogoIcon ||
	...
	private saveShowAllMenuItemsSetting() {
		try {
			localStorage.setItem(
				this.SHOW_ALL_MENU_ITEMS_KEY,
				this.showAllMenuItems.toString(),
			);
		} catch (error) {
