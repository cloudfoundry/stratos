...
function isEndpointListDetailsComponent(
	obj: unknown,
): EndpointListDetailsComponent | null {
	return obj &&
		typeof obj === "object" &&
		"isEndpointListDetailsComponent" in obj &&
		obj.isEndpointListDetailsComponent
		? (obj as EndpointListDetailsComponent)
		: null;
}
...
function combineCreateVisibles(
	...
	if (createVisiblesFns.length === 0) {
		...
		return (row$: Observable<EndpointModel>) => {
			const createVisibles = createVisiblesFns.map((createVisible) =>
				createVisible(row$),
			);
...
export class EndpointListHelper {
	...
	endpointActions(includeSeparators = false): IListAction<EndpointModel>[] {
		...
		const customActions = entityCatalog
			.getAllEndpointTypes()
		...
		return [
			{
				action: (item) => {
					...
					const message2 = item.local
						? `This will also update your local configuration.`
						: "";
					...
					this.confirmDialog.open(confirmation, () => {
						const obs$ =
							stratosEntityCatalog.endpoint.api.disconnect<ActionState>(
								item.guid,
								item.cnsi_type,
							);
				...
				createVisible: (row$: Observable<EndpointModel>) =>
					combineLatest([
						...
						map(([isAdmin, row]) => {
							...
							return (
								isConnected &&
								(!row.system_shared_token ||
									(row.system_shared_token && isAdmin))
							);
						}),
					),
			},
					...
					return combineLatest([
						...
						map(([userEndpointsEnabled, profile, row]) => {
							if (
								...
								!row.creator.system &&
								profile.userName !== row.creator.name
							) {
								...
								const endpoint = entityCatalog.getEndpoint(
									row.cnsi_type,
									row.sub_type,
								);
								const ep = endpoint
									...
									: { unConnectable: false };
								return (
									!ep.unConnectable && row.connectionStatus === "disconnected"
								);
							}
						}),
					);
			...
			},
			{
				action: (item) => {
					...
					this.confirmDialog.open(confirmation, () => {
						const obs$ =
							stratosEntityCatalog.endpoint.api.unregister<ActionState>(
								item.guid,
								item.cnsi_type,
							);
						this.handleAction(obs$, () => {
				...
				createVisible: (row$: Observable<EndpointModel>) => {
					...
					return combineLatest([
						...
						this.currentUserPermissionsService.can(
							StratosCurrentUserPermissions.EDIT_ENDPOINT,
						),
						row$,
					]).pipe(
	...
	private handleAction(
		obs$: Observable<ActionState>,
		handleChange: ([o, n]: [ActionState, ActionState]) => void,
	) {
		const disSub = obs$.pipe(pairwise()).subscribe(([oldVal, newVal]) => {
	...
	createEndpointDetails(
		listDetailsComponent: typeof EndpointListDetailsComponent,
		container: ViewContainerRef,
		componentFactoryResolver: ComponentFactoryResolver,
	): EndpointDetailsContainerRefs {
		const componentFactory =
			componentFactoryResolver.resolveComponentFactory<EndpointListDetailsComponent>(
				listDetailsComponent as any,
			);
		const componentRef =
			container.createComponent<EndpointListDetailsComponent>(componentFactory);
		...
		if (!component) {
			console.warn(
				`Attempted to create a non-endpoint list details component "${listDetailsComponent}"`,
			);
	...
	destroyEndpointDetails(refs: EndpointDetailsContainerRefs) {
		if (refs.componentRef?.destroy) {
			refs.componentRef.destroy();
		}
		if (refs.endpointDetails?.clear) {
