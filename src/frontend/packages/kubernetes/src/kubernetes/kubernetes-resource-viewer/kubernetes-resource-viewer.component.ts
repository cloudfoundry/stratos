...
import { type Portal, TemplatePortal } from "@angular/cdk/portal";
import {
	type AfterViewInit,
	Component,
...
import type { DeleteActionState } from "../../../../store/src/reducers/api-request-reducer/types";
import type {
	IFavoriteMetadata,
	UserFavorite,
...
export interface KubernetesResourceViewerConfig {
	title: string;
	analysis?: { alerts?: Record<string, unknown> };
	resource$: Observable<BasicKubeAPIResource>;
	resourceKind: string;
	component?: Type<PreviewableComponent>;
	definition?: KubeResourceEntityDefinition;
}

interface KubernetesResourceViewerResource {
	raw: BasicKubeAPIResource;
...
@Component({
	...
	standalone: true,
	imports: [RouterLink, AsyncPipe, DatePipe, SidepanelPreviewComponent],
})
export class KubernetesResourceViewerComponent
	implements PreviewableComponent, OnDestroy, AfterViewInit
{
	...
	private analysis?: { alerts?: Record<string, unknown> };
	public alerts: unknown;

	public favorite: UserFavorite<IFavoriteMetadata>;

	// Custom component
	@ViewChild("customComponent", { read: ViewContainerRef, static: false })
	customComponentContainer: ViewContainerRef | undefined;
	...
	createCustomComponent(): void {
		...
		if (this.component && this.customComponentContainer) {
			const factory: ComponentFactory<PreviewableComponent> =
				this.resolver.resolveComponentFactory(this.component);
			this.componentRef =
	...
	ngAfterViewInit(): void {
		...
		setTimeout(() => {
			this.headerContent = new TemplatePortal(
	...
	setProps(props: KubernetesResourceViewerConfig): void {
		...
		this.resource$ = props.resource$.pipe(
			filter((item) => !!item),
			map((item) => {
				const resource: KubernetesResourceViewerResource =
					{} as KubernetesResourceViewerResource;
				const newItem: Record<string, unknown> = {};
				const itemWithDynamicProps = item as unknown as Record<string, unknown>;
				...
				resource.jsonView = newItem as unknown as KubeAPIResource;

				const fallback = (itemWithDynamicProps._metadata || {}) as unknown as Record<string, unknown>;

				const ts = item.metadata
					? item.metadata.creationTimestamp
					: fallback.creationTimestamp;
				resource.age = formatDistance(new Date(ts as string), new Date());
				resource.creationTimestamp = ts as string;
				...
				resource.kind =
					(itemWithDynamicProps.kind as string) || (fallback.kind as string) || props.resourceKind;
				const metadataWithSelfLink = item.metadata as unknown as Record<string, unknown>;
				resource.apiVersion =
					(itemWithDynamicProps.apiVersion as string) ||
					(fallback.apiVersion as string) ||
					this.getVersionFromSelfLink(metadataWithSelfLink.selfLink as string);
		...
		this.hasPodMetrics$ =
			props.resourceKind === "pod"
				? this.resource$.pipe(
						switchMap((resource) =>
							this.endpointsService.hasMetrics(
	...
	private getEndpointId(res: {
		endpointId?: string;
		metadata?: { kubeId?: string };
	}): string | undefined {
		return (
	...
	private setFavorite(
		defn: KubeResourceEntityDefinition,
		item: BasicKubeAPIResource,
	): void {
		if (defn) {
			const entityDefn = entityCatalog.getEntity(
	...
	public deleteWarn(): void {
		...
		const defnRaw = this.data.definition as unknown as Record<string, unknown>;
		const defn = (defnRaw?.definition ||
			this.data.definition) as KubeResourceEntityDefinition;
		...
		this.confirmDialog.openWithCancel(
			confirmation,
			() => {
				const catalogEntity = entityCatalog.getEntityFromKey(
					entityCatalog.getEntityKey(KUBERNETES_ENDPOINT_TYPE, defn.type),
				) as StratosCatalogEntity<
					IFavoriteMetadata,
					BasicKubeAPIResource,
				...
				catalogEntity.api
					...
					.pipe(entityDeleted(), first())
					.subscribe((result: DeleteActionState) => {
						const msg = result.error
							? `Could not delete resource: ${result.message}`
							: `Deleted resource '${this.data.resource.metadata.name}'`;
