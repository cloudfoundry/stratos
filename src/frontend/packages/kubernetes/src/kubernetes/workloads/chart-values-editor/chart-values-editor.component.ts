...
export class ChartValuesEditorComponent
	implements OnInit, OnDestroy, AfterViewInit
{
	...
	public model: Monaco.editor.ITextModel | null = null;
	...
	// JSON Schema
	public schema: Record<string, unknown> | null = null;

	public hasSchema = false;

	// Data shown in the form on load
	public initialFormData: Record<string, unknown> = {};
	...
	public chartValues: Record<string, unknown> = {};
	...
	@ViewChild("monacoEditor", { read: ElementRef, static: false })
	monacoEditor: ElementRef;

	@ViewChild("schemaForm", { static: false }) schemaForm:
		| TailwindJsonSchemaFormComponent
		| undefined;
	...
	private init() {
		...
		const schema$ = this.httpClient
			.get<Record<string, unknown> | null>(this.schemaUrl)
			.pipe(catchError(() => of(null)));
		const values$: Observable<string | null> = this.httpClient
			.get(this.valuesUrl, { responseType: "text" })
			.pipe(catchError(() => of(null)));
		...
		this.loading$ = combineLatest(
			schema$,
			values$,
			toObservable(this.monacoLoaded),
		).pipe(
			filter(
				([schema, values, loaded]: [
					...
					string | null,
					boolean,
				]) => schema !== undefined && values !== undefined && loaded,
			),
			tap(
				([schema, values, _loaded]: [
					Record<string, unknown> | null,
					string | null,
					boolean,
				]) => {
					...
					if (values !== null) {
						...
						this.chartValues = yaml.load(values, { json: true }) as Record<
							string,
							unknown
						>;
	...
	formChanged(data: Record<string, unknown>) {
		this.formData = data;
	}

	// The edit mode has changed (form or editor)
	editModeChanged(mode: { value: EditorMode }) {
		...
		if (this.mode === EditorMode.CodeEditor) {
			...
			if (!this.yamlError) {
				const codeYaml = yaml.load(this.code || "{}", { json: true }) as Record<
					string,
					unknown
				>;
	...
	onMonacoInit(editor: Monaco.editor.IStandaloneCodeEditor) {
		...
		const req = (window as { require?: unknown }).require as
		...
		this.themeSub = this.themeService
			.getTheme()
			.subscribe((theme: { styleName: string }) => {
	...
	private updateModel() {
		...
		this.model = {
			language: "yaml",
			uri: monaco?.Uri.parse(this.getSchemaUri()),
	...
	registerSchema(schema: Record<string, unknown>) {
		const monaco = (window as { monaco?: typeof Monaco }).monaco;
	...
	public getValues(): object {
		...
		return this.mode === EditorMode.JSonSchemaForm
			? diffObjects(this.formData, this.chartValues)
			: (yaml.load(this.code) as object);
	}

	public copyValues() {
		const confirm =
			this.mode === EditorMode.JSonSchemaForm ||
			(this.mode === EditorMode.CodeEditor && this.code.length > 0);
		if (confirm) {
	...
	private doCopyReleaseValues() {
		if (this.mode === EditorMode.JSonSchemaForm) {
			this.initialFormData = this.releaseValues as unknown as Record<string, unknown>;
		} else {
			this.code = yaml.dump(this.releaseValues);
		}
	}
	...
	getDiff(userValues: unknown): string {
		let code = yaml.dump(
			diffObjects(userValues as Record<string, unknown>, this.chartValues),
		);
		if (code.trim() === "{}") {
			code = "";
		}
