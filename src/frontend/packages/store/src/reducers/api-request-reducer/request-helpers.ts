...
export function getEntityRequestState(state: IRequestDataState, action: SingleEntityAction): RequestInfoState {
    const { entityType: entityKey, guid } = action;
...
export function setEntityRequestState(state: IRequestDataState, requestState, { entityType: entityKey, guid }: IRequestAction) {
    const newState = {
        [entityKey]: {
            [guid]: {
                ...requestState
...
export function createRequestStateFromResponse(response: NormalizedResponse, state: IRequestDataState) {
    ...
    Object.keys(entities).forEach(entityKey => {
        Object.keys(entities[entityKey]).forEach(guid => {
            const entState = getEntityRequestState(state, { entityType: entityKey, guid } as SingleEntityAction);
            ...
            newState = setEntityRequestState(newState, entState, { entityType: entityKey, guid } as IRequestAction);
        });
    });
    return newState;
}
