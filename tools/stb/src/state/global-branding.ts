...
export function mergeScenes(perScene: { scene: string; model: BrandingModel }[]): NavNode[] {
  ...
  for (const { scene, model } of perScene) {
    for (const n of model.nodes) {
      nodes.push({ snapshotId: n.snapshotId, scene, name: n.name, description: n.description, facets: n.facets, ...(n.role ? { role: n.role } : {}), ...(n.roledescription ? { roledescription: n.roledescription } : {}) });
    }
  }
  return nodes;
}
