...
(function () {
    ...
    return helpers.createReqAndSession(null, username, password)
      ...
      .then(function (response) {
        ...
        _.forEach(cnsis, function (cnsi) {
          var list = helpers.getCNSIs()[cnsi.cnsi_type];
          if (!list) {
            fail('Unknown cnsi');
          }
  ...
  function _resetAllCNSI(req, registerMultipleCf) {
    return new Promise(function (resolve, reject) {
      _removeAllCnsi(req).then(function () {
        var promises = [];
        _.forEach(helpers.getCNSIs(), function (cnsisPerType, cnsiType) {
          if (registerMultipleCf) {
            if (_.keys(cnsisPerType).length === 1) {
              // duplicates the current definition
              var key = _.keys(cnsisPerType)[0];
              cnsisPerType[key + '_test'] = _.clone(cnsisPerType[key]);
              cnsisPerType[key + '_test'].name += ' Copy';
            }
          } else {
            var firstKey = Object.keys(cnsisPerType)[0];
            var firstCf = cnsisPerType[firstKey];
            cnsisPerType = {};
            cnsisPerType[firstKey] = firstCf;
          }
          _.forEach(cnsisPerType, function (cnsi) {
            promises.push(helpers.sendRequest(req, {method: 'POST', url: 'pp/v1/register/' + cnsiType}, null, cnsi.register));
          });
        });
