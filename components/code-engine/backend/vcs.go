...
import (
	...
	"github.com/hpcloud/portal-proxy/components/core/backend/repository/vcstokens"
...
func (p *portalProxy) checkVcsToken(c echo.Context) error {
	...
	if err != nil {
		...
		return interfaces.NewHTTPShadowError(http.StatusInternalServerError, msg, msg+": %v", err)
	}
