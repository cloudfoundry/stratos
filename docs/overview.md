...
The Startos UI Console provides a web-based UI to allow developers and administrators to manege their applications and cloud foundry deployment(s).
...
* Web UI - Single-page AngularJS web application providing the front-end application that runs in the user's browser.
...
Administrators used the 'Endpoints Dashboard' within the Console to add new endpoints to the Console. All users are then able to connect to these endpoints by providing their credentials. The Console will use these credentials to communicate with the UAA for the given endpoint (typically Cloud Foundry) and obtain a refresh and access token. These tokens are encrypted and stored in the Endpoint Datastore.
